...
export class ChatService {
    constructor(
        ...
        @InjectRepository(SessionChatEntity) private sessionChatRepository: Repository<SessionChatEntity>
    ) { }
    ...
    async sendMessage(sender: string, receiver: string, message: string) {
        let sessionChat = await this.sessionChatRepository.findOne({ where: { sender, receiver } });
        if (!sessionChat) {
