...
export class FeedbackController {
  ...
  @Get('/uniqueFeedback/:evaluated/:evaluator')
  async findByEvaluatorAndEvaluated(
    @Param('evaluated') evaluatedId: string,
    @Param('evaluator') evaluatorId: string,
  ): Promise<any[]> {
    return this.feedbackService.findByEvaluatorAndEvaluated(
      evaluatedId,
      evaluatorId,
    );
  }
